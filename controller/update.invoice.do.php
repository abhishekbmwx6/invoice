...
if($_POST && $_POST["contact"]){
  ...
  $invoice_id = base64_decode($_GET['id']);
    ...
    foreach($_POST["price"] as $key=>$val){
      ...
      echo $query = "delete from invoice where invoice_id='".$invoice_id."'";
   ...
   ,address="'.$_POST['address'].'",
    ...
    if(!isset($user_id) || $user_id==0){
        $query = "select user_id from user where contact='".$_POST['contact']."'";
        $stmt = $conn->prepare($query); 
        $stmt->execute();
    ...
    echo "<br>";
    //insert Invoice
  echo  $query ='INSERT INTO `invoice` SET user_id="'.$user_id.'"
   , purches_date=now()
  ...
  //  $invoice_id = $conn->lastInsertId();
   ...
   foreach($_POST["product"] as $key => $value){
    echo "<br>";
  echo $query ='INSERT INTO `invoice_products` SET invoice_id="'.$invoice_id.'"
    , product_name="'.$value.'"
